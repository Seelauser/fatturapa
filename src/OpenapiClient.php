...
/**
 * HTTP client for the Openapi.com SDI service.
 *
 * Documented API (https://console.openapi.com/apis/sdi/documentation):
 *   - Base URL: https://sdi.openapi.it (production), https://test.sdi.openapi.it (sandbox)
 *   - Auth: Bearer token
 *   - POST /invoices            — send invoice XML; returns a uuid
 *   - GET  /invoices/{uuid}     — invoice data incl. status
 *   - Webhooks configured via POST /api_configurations during onboarding
 *
 * NOTE: confirm exact payload field names against your Openapi console during
 * onboarding; the endpoints/paths are settings-overridable for this reason.
 */
class CRM_Fatturapa_OpenapiClient implements CRM_Fatturapa_SdiTransportInterface {

  const BASE_PRODUCTION = 'https://sdi.openapi.it';
  const BASE_TEST = 'https://test.sdi.openapi.it';
  ...
  public function __construct(string $token, bool $testMode = TRUE, ?Client $http = NULL, ?callable $logger = NULL, ?callable $sleeper = NULL) {
    $this->token = $token;
    $this->baseUrl = $testMode ? self::BASE_TEST : self::BASE_PRODUCTION;
    $this->http = $http ?: new Client(['timeout' => self::TIMEOUT]);
    $this->logger = $logger ?: ['CRM_Fatturapa_Log', 'sdi'];
    $this->sleeper = $sleeper ?: 'sleep';
  }

  /**
   * Build a client from extension settings + token from environment.
   */
  public static function createFromSettings(): self {
    $token = CRM_Fatturapa_Env::get('OPENAPI_TOKEN');
    if (!$token) {
      throw new CRM_Fatturapa_TransportException(
        'OPENAPI_TOKEN is not configured (set it in the extension .env file or as SDI_OPENAPI_TOKEN constant in civicrm.settings.php)');
    }
    $testMode = Civi::settings()->get('sdi_mode') !== 'production';
  ...
  public function sendInvoice(string $xml, array $meta = []): array {
    ...
    if (!$uuid) {
      throw new CRM_Fatturapa_TransportException(
        'Openapi response did not contain an invoice uuid: ' . json_encode($response));
    }
   ...
   *
   * Retries on network errors and 5xx; 4xx fails immediately (the request
   * will not get better by repeating it).
   */
  private function request(string $method, string $path, array $options, array $logContext = []): array {
    ...
    while (++$attempt <= self::MAX_ATTEMPTS) {
      try {
        ...
        if ($status >= 400 && $status < 500) {
          throw new CRM_Fatturapa_TransportException("Openapi rejected the request (HTTP $status): $body", $status, $e);
        }
        $lastError = $e;
      }
    ...
    throw new CRM_Fatturapa_TransportException(
      'Openapi request failed after ' . self::MAX_ATTEMPTS . ' attempts: ' . ($lastError ? $lastError->getMessage() : 'unknown'),
      0, $lastError);
  }
